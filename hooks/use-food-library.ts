...
export function useFoodLibrary(): UseFoodLibraryReturn {
  ...
  const searchFoodItems = useCallback(
    async (params: FoodLibrarySearchParams) => {
      try {
        setLoading(true);
        setError(null);
  ...
  // 本地模糊匹配
  const matchFoodItems = useCallback(
  ...
  const parseText = useCallback(
    async (text: string): Promise<ParseContext | null> => {
      try {
        // 直接在前端调用本地 parseTextContext
        const context = parseTextContext(text);

        // 检查每个营养段落中的食物是否已存在于库中
        if (context.nutritionSegments.length > 0) {
          const updatedSegments = await Promise.all(
            context.nutritionSegments.map(async (segment) => {
              if (segment.canAddToLibrary) {
                // 检查食物是否已存在
                const matches = await matchFoodItems(segment.foodName, 1);
                const hasExactMatch = matches.length > 0;

                // 如果已存在完全匹配的食物，则不应该显示添加提示
                return {
                  ...segment,
                  canAddToLibrary: !hasExactMatch,
                };
              }
              return segment;
            })
          ...
          return {
            ...context,
            nutritionSegments: updatedSegments,
            hasNutritionData: updatedSegments.some(
              (segment) => segment.canAddToLibrary
            ),
          };
        }
  ...
  const incrementUsage = useCallback(
    async (foodItemId: string): Promise<boolean> => {
      try {
  ...
  const exportFoodItems = useCallback(async (options: ExportOptions = {}) => {
    try {
      ...
      if (options.includeUsageStats)
        searchParams.set("includeUsageStats", "true");
      ...
      const response = await fetch(
        `/api/db/food-library/export?${searchParams}`,
        {
          headers: {
            ...getAuthHeaders(),
          },
      ...
      const filename =
        ...
        `food-library-${new Date().toISOString().split("T")[0]}.${
          options.format || "json"
        }`;
  ...
  // 导入饮食库数据
  const importFoodItems = useCallback(
    async (
      data: any,
      options: ImportOptions = {}
    ): Promise<ImportResult | null> => {
  ...
  const validateImportData = useCallback(
    async (data: any): Promise<ValidationResult | null> => {
      try {
        const response = await fetch("/api/db/food-library/import", {
          method: "PUT",
          headers: {
